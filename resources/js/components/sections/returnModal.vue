<template>
  <Modal :show="show" @close="$emit('close')">
    <template v-slot:body>
      <div class="returnModal grid grid-cols-4">
        <div class="col-span-4 md:col-span-3 p-9 flex flex-wrap">
          ...
          <div class="w-full md:w-4/6 pr-8">
            <h3 class="text-xl text-gray-700 font-semibold">Select a return date</h3>
          ...
          <div v-if="selectedDate" class="w-full md:w-2/6 pr-4">
            <h3 class="text-xl text-gray-700 font-semibold">Select a return time</h3>
            <p class="mb-3">
        ...
        <div class="col-span-4 md:col-span-1 bg-gray-300 p-9">
          ...
          <sub>The full address will be received by email and is the same used to pick up the book</sub>
          <hr>
          <h2>Return date:</h2>
...
export default {
  ...
  data () {
    return {
      ...
      maxDate: DateTime.now().plus({ weeks: 3 }),
      selectedDate: null,
  ...
  methods: {
    ...mapActions('ledge', ['returnRequest']),
    ...
    async onSendRequest () {
      ...
      const payload = {
        ledgeId: this.selectedBook.ledgeId,
        return_date: DateTime.fromFormat(this.selectedDate, 'cccc, d LLLL').set({ hour: timeslotHour }).toFormat('yyyy-LL-dd T')
      }

      try {
        await this.returnRequest(payload)
        this.$emit('close')
        Swal.fire({
          type: 'success',
          title: 'Good Job!',
