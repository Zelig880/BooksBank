<template>
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 ml-8">
      ...
      <div class="flex items-center" v-if="selectedTypeIndex === 2">
        <label>
          {{ $t( 'bookshelfAdd-step3-price' ) }}
        </label>
        <input type="number" step="0.01" value="0" class="BookshelfAddBookshelf-field rounded-md border-2 pl-8 pr-6 py-2 block mb-4 mt-1" @keyup="$emit('update:price', $event.target.value)" @change="$emit('update:price', $event.target.value)">
      </div>
    </div>
  </div>
</template>
